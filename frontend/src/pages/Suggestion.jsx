                ...
                <div className="success-message-box">
                  ...
                  <div className="action-buttons-group">
                    <button
                      ...
                      onClick={() => {
                        setSubmitted(false);
                        setFormData({ contributorName: "", dob: "", companyName: "", details: "", attachments: [] });
                      }}
                    >
            ...
            <div className={`form-container ${submitted ? "form-hidden" : ""}`}>
              <form onSubmit={handleSubmit} className="suggestion-form">
                ...
                <div className="form-group">
                  <label><User size={16} /> Contributor Name (Optional)</label>
                  <input type="text" placeholder="Anonymous" value={formData.contributorName} onChange={(e) => setFormData({ ...formData, contributorName: e.target.value })} />
                </div>

                <div className="form-group">
                  <label><Calendar size={16} /> Date of Birth <span className="required">*</span></label>
                  <div className="date-input-wrap">
                    <input type="text" required placeholder="dd/mm/yyyy" value={formData.dob} onChange={(e) => setFormData({ ...formData, dob: e.target.value })} />
                    <div className="calendar-picker-trigger">
                      <Calendar size={20} className="cal-icon" />
                      <input type="date" className="hidden-date-input" onChange={handleDateSelect} />
                    </div>
                  </div>
                </div>

                <div className="form-group">
                  <label><Building size={16} /> Company Name (Optional)</label>
                  <input type="text" placeholder="Enter company name" value={formData.companyName} onChange={(e) => setFormData({ ...formData, companyName: e.target.value })} />
                </div>

                <div className="form-group">
                  <label><MessageSquare size={16} /> Proposal/Contribution Details <span className="required">*</span></label>
                  <textarea required rows="4" placeholder="Tell us what you are thinking..." value={formData.details} onChange={(e) => setFormData({ ...formData, details: e.target.value })}></textarea>
                </div>

                <div className="form-group">
                  <label><Paperclip size={16} /> Attachments ({formData.attachments.length}/10)</label>
                  ...
                  <small>Supported formats: PDF, Word, Excel, PNG, JPG (Max 10 files).</small>
                </div>
