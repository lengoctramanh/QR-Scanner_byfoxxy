...
const register = async (req, res) => {
  try {
    const { 
      ...
      fullName, dob, gender, 
      businessName, taxId, hqAddress, industry, website, 
      supplierType, region, scale, serviceTypes 
    } = req.body;
    ...
    if (existingUsers.length > 0) {
      return res.status(400).json({ success: false, message: 'Account with this email or phone number already exists!' });
    }
    ...
    if (userRole === 'user') {
      ...
      await db.query(
        ...
        [brandId, accountId, businessName, businessName, taxId, hqAddress, industry, website || null] 
      );
    } 
      ...
      await db.query(
        ...
        [supplierId, accountId, fullName, taxId || null, supplierType, region, scale || null, JSON.stringify(serviceTypes || [])]
      );
    }
